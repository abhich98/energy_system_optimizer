...
class OptimizationCostCalculator:
	...
	def _detect_mode(df: pd.DataFrame) -> str:
		...
		if {"scenario", "probability", "grid_import_rt", "grid_export_rt"}.issubset(cols):
	...
	def _deterministic_breakdown(
		...
		for battery_id, deg_cost in degradation_costs.items():
			...
			throughput_sum = self._sum_series(df, charge_col) + self._sum_series(df, discharge_col)
	...
	def _stochastic_expected_breakdown(
		...
		for col in ["import_price_ahead", "export_price_ahead", "expected_import_price_rt", "expected_export_price_rt"]:
			if col not in df.columns:
				raise ValueError(f"Stochastic expected cost calculation needs '{col}' column")
		...
		import_rt_cost = float(
			(
				pd.to_numeric(df["expected_import_price_rt"], errors="coerce").fillna(0.0)
				* pd.to_numeric(df["expected_grid_import_rt"], errors="coerce").fillna(0.0)
			).sum()
			* self.dt_hours
		)
		...
		export_rt_revenue = float(
			(
				pd.to_numeric(df["expected_export_price_rt"], errors="coerce").fillna(0.0)
				* pd.to_numeric(df["expected_grid_export_rt"], errors="coerce").fillna(0.0)
			).sum()
			* self.dt_hours
			)
		...
		for battery_id, deg_cost in degradation_costs.items():
			...
			charge_col, discharge_col = f"expected_{battery_id}_charge", f"expected_{battery_id}_discharge"
			throughput_sum = self._sum_series(df, charge_col) + self._sum_series(df, discharge_col)
			battery_deg[battery_id] = deg_cost * throughput_sum * self.dt_hours
	...
	def _stochastic_scenario_breakdown(
		...
		for scenario_id, scenario_df in df.groupby("scenario", sort=True):
			...
			import_ahead_cost = float(
				(
					pd.to_numeric(scenario_df["import_price_ahead"], errors="coerce").fillna(0.0)
					* pd.to_numeric(scenario_df["grid_import_ahead"], errors="coerce").fillna(0.0)
				).sum()
				* self.dt_hours
			)
			import_rt_cost = float(
				(
					pd.to_numeric(scenario_df["import_price_rt"], errors="coerce").fillna(0.0)
					* pd.to_numeric(scenario_df["grid_import_rt"], errors="coerce").fillna(0.0)
				).sum()
				* self.dt_hours
			)
			...
			export_ahead_revenue = float(
				(
					pd.to_numeric(scenario_df["export_price_ahead"], errors="coerce").fillna(0.0)
					* pd.to_numeric(scenario_df["grid_export_ahead"], errors="coerce").fillna(0.0)
				).sum()
				* self.dt_hours
			)
			export_rt_revenue = float(
				(
					pd.to_numeric(scenario_df["export_price_rt"], errors="coerce").fillna(0.0)
					* pd.to_numeric(scenario_df["grid_export_rt"], errors="coerce").fillna(0.0)
				).sum()
				* self.dt_hours
			)
			...
			for battery_id, deg_cost in degradation_costs.items():
				...
				if charge_col not in scenario_df.columns or discharge_col not in scenario_df.columns:
					warnings.append(
					...
					continue
				throughput_sum = self._sum_series(scenario_df, charge_col) + self._sum_series(
					scenario_df, discharge_col
				)
				scenario_battery_deg[battery_id] = deg_cost * throughput_sum * self.dt_hours
			...
			for battery_id in expected_battery_deg:
				expected_battery_deg[battery_id] += (
	...
	def calculate_from_dataframe(
		...
		match mode:
			...
			case _:
				raise ValueError(f"Unsupported mode '{mode}'. Supported modes: deterministic, stochastic_expected, stochastic_scenarios.")
