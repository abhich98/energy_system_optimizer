...
def read_price_table(
    ...
    return ensure_unique_date_rows(
        prices, ["Energy price (EUR/MWh)", "Energy price (EUR/kWh)"]
    )
...
def build_dataset(config: dict[str, Any], config_path: Path) -> pd.DataFrame:
    ...
    with h5py.File(hdf5_path, "r") as h5_file:
        ...
        pv["PV generation (kW)"] *= eval(
            config["PV_ratio"]
        )  # Scale PV generation to household load
    ...
    dataset["Energy price (EUR/kWh)"] = dataset["Energy price (EUR/kWh)"].ffill()
    dataset["Energy price (EUR/kWh)"] += config["prices_fixed_comp_euro_kwh"]
    dataset["Energy price (EUR/kWh)"] = dataset["Energy price (EUR/kWh)"].clip(
        lower=0.0
    )  # Ensure no negative prices after adding the fixed component
    dataset["Energy price (EUR/MWh)"] = dataset["Energy price (EUR/kWh)"] * 1000.0
    ...
    dataset["Consumption (pu)"] = dataset["Consumption (kW)"] / dataset[
        "Consumption (kW)"
    ].max(skipna=True)
...
def write_dataset(
    dataset: pd.DataFrame, config: dict[str, Any], output_path: Path
) -> None:
